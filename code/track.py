...
def putText(im, label, coor):
    im = cv2.putText(im, label, coor, cv2.FONT_HERSHEY_PLAIN, 4, (100, 50, 150), 3, cv2.LINE_AA)
    return im
...
def draw_boxes(img, bbox, identities=None, categories=None, names=None, offset=(0, 0)):
    for i, box in enumerate(bbox):
        ...
        ids = int(identities[i]) if identities is not None else 0
        
        color = compute_color_for_labels(ids)
    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset): #for every frame
        ...
        for i, det in enumerate(pred): #for each detection in this frame
            ...
            if len(tracked_dets)>0:
                ...
                putText(im0, label='Adeel Intizar', coor = (850, 90))
                ids = [int(i) for i in identities]
                putText(im0, label = str(max(ids)), coor = (1725, 90))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--classes', nargs='+', type=int,
                        default=[2, 5, 7, 67], help='filter by class') #80 classes in COCO dataset
